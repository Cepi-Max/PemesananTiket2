...
class KotaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Ambil data kota dengan pencarian dan pagination
        $kota = Kota::when($search, function ($query) use ($search) {
                return $query->where('nama_kota', 'like', "%{$search}%");
            })
            ->paginate(5); // Atur jumlah item per halaman sesuai kebutuhan
