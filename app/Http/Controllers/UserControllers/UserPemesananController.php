...
class UserPemesananController extends Controller
{
    ...
    public function chartPerMaskapai()
    {
        $data = DB::table('penerbangan')
            ...
            ->select('pesawat.nama_maskapai', DB::raw('COUNT(detail_penumpang.id) as total_penumpang'))
            ->groupBy('pesawat.nama_maskapai')
            ->get();
    
        return view('user-pages.chart-maskapai', compact('data'));
    }
