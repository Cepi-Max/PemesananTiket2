...
<body class="bg-white text-gray-800 font-sans">
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <form action="{{ route('penerbangan.search') }}" method="GET" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="bandara_asal" id="bandara_asal"
                        ...
                        @foreach ($bandaraList as $bandara)
                            <option value="{{ $bandara->nama_bandara }}" 
                                {{ request('bandara_asal') !== null && request('bandara_asal') == $bandara->nama_bandara ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="bandara_tujuan" id="bandara_tujuan"
                        ...
                        @foreach ($bandaraList as $bandara)
                            <option value="{{ $bandara->nama_bandara }}" 
                                {{ request('bandara_tujuan') !== null && request('bandara_tujuan') == $bandara->nama_bandara ? 'selected' : '' }}>
                                {{ $bandara->nama_bandara }}
                            </option>  
                        @endforeach
                    </select>
                ...
                {{-- Kelas Penerbangan --}}
                <div>
                    <label for="namaKelas" class="block text-sm font-medium text-gray-700 mb-1">
                        Kelas Penerbangan
                    </label>
                    <select name="namaKelas" id="namaKelas"
                        class="block w-full rounded-lg border border-gray-300 bg-white py-2 px-3 text-sm text-gray-700 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="" disabled {{ request('namaKelas') ? '' : 'selected' }}>Pilih kelas</option>
                        @foreach ($kelasList as $class)
                            <option value="{{ $class->nama_kelas }}" 
                                {{ request('namaKelas') !== null && request('namaKelas') == $class->nama_kelas ? 'selected' : '' }}>
                                {{ $class->nama_kelas }}
                            </option>
                        @endforeach
                    </select>
        ...
        <section class="md:col-span-3">
            ...
            @endforeach
            @else
                <div class="bg-white border rounded-xl shadow-sm p-8 mb-4">
                    <div class="flex flex-col items-center justify-center text-center">
                        <div class="w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center mb-4">
                            <h1 class="text-6xl font-bold text-gray-500">!</h1>
                        </div>
                        <h3 class="text-xl font-bold text-gray-800 mb-2">Penerbangan Tidak Ditemukan</h3>
                        <p class="text-gray-600 max-w-md mb-6">Maaf, penerbangan yang Anda cari saat ini tidak tersedia. Silakan coba dengan kriteria pencarian yang berbeda.</p>
                        <a href="{{ route('penerbangan.search') }}" class="px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2">
